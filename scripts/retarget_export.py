...
def retarget_and_export(vrm_path, anim_fbx_path, output_fbx_path):
    ...
    # Zero out FBX armature's object transform before binding.
    # The original pipeline (direct slot assignment) transfers the FBX action's
    # bone-level and object-level fcurves separately. Bone keyframes are in
    # armature space (no object rotation). Object fcurves (root motion) set the
    # armature's world position/rotation directly.
    # Without zeroing, the visual bake captures bones in world space (including
    # the FBX's object rotation), breaking armature_z_override configs.
    # With zeroing, bones are baked in armature space, matching slot assignment.
    # Root motion fcurves are saved and re-added after baking.
    fbx_arm.location = (0, 0, 0)
    fbx_arm.rotation_euler = (0, 0, 0)
    fbx_arm.scale = (1, 1, 1)

    # Save and remove object-level fcurves (root motion)
    root_motion_data = []
            ...
            for cb in strip.channelbags:
                for fc in list(cb.fcurves):
                    if 'pose.bones' not in fc.data_path:
                        kf_data = [(kp.co[0], kp.co[1], kp.interpolation) for kp in fc.keyframe_points]
    ...
    # Re-add root motion fcurves (absolute object-level animation)
    # VRM imports as QUATERNION rotation mode, but FBX root motion uses euler.
    # Must switch to euler so the FBX exporter picks up rotation_euler fcurves.
